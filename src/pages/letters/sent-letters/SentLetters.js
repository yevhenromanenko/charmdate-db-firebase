import SentLettersOnePage from "./SentLettersOnePage";

const SentLetter = async (ladyId) => {
    try {
        ...
        let sentEmfArray = [];
        let nextPageUrl = `https://www.charmdate.com/clagt/emf_wm_result.php?adddate_s_m=${formattedStartDate}&adddate_e_m=${formattedEndDate}&womanid=${ladyId}`;
        ...
        while (nextPageUrl) {
            const { results, nextPage } = await SentLettersOnePage(nextPageUrl);

            sentEmfArray.push(results);

            if (nextPage && nextPage.length > 0) {
                nextPageUrl = `https://www.charmdate.com/clagt/emf_wm_result.php?adddate_s_m=${formattedStartDate}&adddate_e_m=${formattedEndDate}&womanid=${ladyId}&page=${nextPage}`;
            } else {
                nextPageUrl = null;
            }
        }
